#' @export plot.choicealpha
#' @export
#' @name plot.choicealpha
#' @method plot choicealpha
#' @title  Plot of the mixing parameter
#' @description Plot of the criterion Q or Qnorm obtained in 
#' output of the function \code{choicealpha}.
#' @param x an object of class \code{choicealpha}.
#' @param norm if TRUE, the criterion Qnorm is plotted. Otherwise, it is Q.
#' @param lty a vector of size 2 with the line types of the two curves. See \link{par}
#' @param pch a vector of size 2 specifying the symbol for the points of the two curves. See \link{par}
#' @param type a vector of size 2 specifying the type of lines of the two curves. See \link{par}
#' @param col a vector of size 2 specifying the colors the two curves. See \link{par}
                             ...
                             legend=NULL,cex=1,...)
{
  ...
  if (is.null(legend))
    ...
    pos <- listpos[order(c(x$Qnorm[1,2],x$Qnorm[length(x$range.alpha),1]),decreasing=TRUE)[1]]
    matplot(x$range.alpha,x$Qnorm,ylim=c(0,1), 
            lty=lty,pch=pch,type=type,col=col,
            xlab=xlab,ylab=ylab,...)
    legend(pos,legend=legend, col=col,
           lty=lty,pch=pch,bty="n",cex=cex)
    mtext(side=3,paste("of ",round(x$Q[1,1]*100,digits=0),"%",sep=""),adj=0,cex=cex,...)
    mtext(side=3,paste("of ",round(x$Q[length(x$range.alpha),2]*100,digits=0),"%",sep=""),adj=1,cex=cex,...)
  }
}